...
public class MainView extends View {
  ...
  private void drawDrawable(
      Canvas canvas, Drawable draw, int startingX, int startingY, int endingX, int endingY) {
  ...
  private void drawCellText(Canvas canvas, int value) {
    ...
    if (value >= 8) paint.setColor(getResources().getColor(R.color.text_white));
    else paint.setColor(getResources().getColor(R.color.text_black));
  ...
  private void drawScoreText(Canvas canvas) {
    ...
    canvas.drawText(
        String.valueOf(game.highScore), sXHighScore + textMiddleHighScore, bodyStartYAll, paint);
    ...
    paint.setColor(getResources().getColor(R.color.text_brown));
    canvas.drawText(
        getResources().getString(R.string.score), sXScore + textMiddleScore, titleStartYAll, paint);
    paint.setTextSize(bodyTextSize);
    paint.setColor(getResources().getColor(R.color.text_white));
  ...
  public void drawLoadButton(Canvas canvas, boolean lightUp) {
    if (lightUp)
      drawDrawable(
          canvas, lightUpRectangle, sXLoad, sYIcons, sXLoad + iconSize, sYIcons + iconSize);
    else
      drawDrawable(
          canvas, backgroundRectangle, sXLoad, sYIcons, sXLoad + iconSize, sYIcons + iconSize);

    drawDrawable(
        canvas,
        getDrawable(R.drawable.ic_action_load),
  ...
  public void drawSaveButton(Canvas canvas, boolean lightUp) {
    if (lightUp)
      drawDrawable(
          canvas, lightUpRectangle, sXSave, sYIcons, sXSave + iconSize, sYIcons + iconSize);
    else
      drawDrawable(
          canvas, backgroundRectangle, sXSave, sYIcons, sXSave + iconSize, sYIcons + iconSize);

    drawDrawable(
        canvas,
        getDrawable(R.drawable.ic_action_save),
  ...
  private void drawTrashButton(Canvas canvas, boolean lightUp) {
    ...
    drawDrawable(
        canvas,
        getDrawable(R.drawable.ic_action_trash),
  ...
  private void drawNewGameButton(Canvas canvas, boolean lightUp) {
    if (lightUp)
      drawDrawable(
          canvas, lightUpRectangle, sXNewGame, sYIcons, sXNewGame + iconSize, sYIcons + iconSize);
    else
      drawDrawable(
          canvas,
          backgroundRectangle,
  ...
  private void drawUndoButton(Canvas canvas, boolean lightUp) {
    if (lightUp)
      drawDrawable(
          canvas, lightUpRectangle, sXUndo, sYIcons, sXUndo + iconSize, sYIcons + iconSize);
    else
      drawDrawable(
          canvas, backgroundRectangle, sXUndo, sYIcons, sXUndo + iconSize, sYIcons + iconSize);
      ...
      for (int yy = 0; yy < ROWS; yy++) {
        ...
        if (currentTile != null) {
          ...
          for (int i = aArray.size() - 1; i >= 0; i--) {
            ...
            if (aCell.getAnimationType() == MainGame.SPAWN_ANIMATION) animated = true;
  ...
  private void drawEndlessText(Canvas canvas) {
    ...
    paint.setColor(getResources().getColor(R.color.text_black));
    canvas.drawText(
        getResources().getString(R.string.endless), startingX, sYIcons - centerText() * 2, paint);
  }
  ...
  private void createEndGameStates(Canvas canvas, boolean win, boolean showButton) {
    ...
    if (win) {
      ...
      paint.setTextAlign(Paint.Align.CENTER);
      canvas.drawText(
          getResources().getString(R.string.game_over), middleX, middleY - centerText(), paint);
    }
  }
  ...
  private void getLayout(int width, int height) {
    ...
    gameOverTextSize =
        Math.min(
            Math.min(
                1000f
                    * ((widthWithPadding - gridWidth * 2)
                        / (paint.measureText(getResources().getString(R.string.game_over)))),
                textSize * 2),
            1000f
                * ((widthWithPadding - gridWidth * 2)
                    / (paint.measureText(getResources().getString(R.string.you_win)))));
