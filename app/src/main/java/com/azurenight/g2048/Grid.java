...
public class Grid {
  ...
  public ArrayList<Cell> getAvailableCells() {
    ...
    for (int xx = 0; xx < field.length; xx++)
      for (int yy = 0; yy < field[0].length; yy++)
        if (field[xx][yy] == null) availableCells.add(new Cell(xx, yy));
  ...
  public Tile getCellContent(Cell cell) {
    if (cell != null && isCellWithinBounds(cell)) return field[cell.getX()][cell.getY()];
    else return null;
  }

  public Tile getCellContent(int x, int y) {
    if (isCellWithinBounds(x, y)) return field[x][y];
    else return null;
  }
  ...
  public void saveTiles() {
    for (int xx = 0; xx < bufferField.length; xx++)
      for (int yy = 0; yy < bufferField[0].length; yy++)
        if (bufferField[xx][yy] == null) undoField[xx][yy] = null;
        else undoField[xx][yy] = new Tile(xx, yy, bufferField[xx][yy].getValue());
  }

  public void prepareSaveTiles() {
    for (int xx = 0; xx < field.length; xx++)
      for (int yy = 0; yy < field[0].length; yy++)
        if (field[xx][yy] == null) bufferField[xx][yy] = null;
        else bufferField[xx][yy] = new Tile(xx, yy, field[xx][yy].getValue());
  }

  public void revertTiles() {
    for (int xx = 0; xx < undoField.length; xx++)
      for (int yy = 0; yy < undoField[0].length; yy++)
        if (undoField[xx][yy] == null) field[xx][yy] = null;
        else field[xx][yy] = new Tile(xx, yy, undoField[xx][yy].getValue());
  }

  public void clearGrid() {
    for (int xx = 0; xx < field.length; xx++)
      for (int yy = 0; yy < field[0].length; yy++) field[xx][yy] = null;
  }

  public void clearUndoGrid() {
    for (int xx = 0; xx < field.length; xx++)
      for (int yy = 0; yy < field[0].length; yy++) undoField[xx][yy] = null;
  }
}
